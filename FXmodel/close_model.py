...
class Close_Model:
    ...
    def __init__(self):
        '''
        this Close model is init only once thru out runtime (it builds the model only once),
        but prediction can be called anytime
        '''
        self.model = None
        self.model_path = "./FXmodel/weights/EURUSD_Close.weights.h5"
        self.__build_model()
    ...
    def __build_model(self):
        ...
        output_close = flow_model(config=ConfigClose(),
                                block_count=2,
                                use_causal_mask=True)(embedded_close)

        #Mix low, high and close together
        low_high_mix = ConcatAndLinear(ConfigLowHigh())(output_low, output_high)

        cross_attention = Blocks(config=ConfigClose(),
                                causal_mask=False)(output_close, low_high_mix)

        flows = flow_model(config=ConfigClose(),
                        block_count=4,
                        use_causal_mask=True)(cross_attention)

        outputs = Linear(ConfigClose())(flows)
    ...
    def predict(self, **kwargs):
        ...
        pe = Get_Position(m, ConfigClose.d_model)
        ...
        embedded_high = self.model.layers[3](high)
        output_high = self.model.layers[6](embedded_high + pe)

        embedded_close = self.model.layers[4](close)
        output_close = self.model.layers[7](embedded_close + pe)

        low_high_mix = self.model.layers[8](output_low, output_high)

        cross_attention = self.model.layers[9](output_close, low_high_mix)
